...
def u_solution (x, y, i, j, E, W, N, S, detar_x, detar_y, Re, uu, vv, alpha_u):
    ...
    if N == 0:
        b += -detar_x / detar_y / Re - detar_x * max(-(vv[y+1, x]+vv[y+1,x-1])/2, 0)
...
def v_solution (x, y, i, j, E, W, N, S, detar_x, detar_y, Re, vv, uu, alpha_v):
    ...
    a_w[y, x] = -detar_y / detar_x / Re - detar_y * max((uu[y, x-1]+uu[y, x])/2, 0)
...
def p_add_solution (x, y, i_u, j_u, i_v, j_v, E, W, N, S, detar_x, detar_y, Re, uu, vv, alpha_u, alpha_v, alpha_p):
    ...
    if x == n - 1:
        a_e[y, x] = detar_x_u * detar_y_u / detar_t \
                + detar_y_u * (max(-(uu[y,x])/2, 0)) \
                + detar_x_u* (max((vv[y+1,x])/2, 0) + max(-(vv[y,x])/2, 0)) \
        ...
        a_e[y, x] = detar_x_u * detar_y_u / detar_t \
                    + detar_y_u * (max(-(uu[y, x]) / 2, 0)) \
                    + detar_x_u * (max(-(vv[y, x]) / 2, 0)) \
                    + 2 / Re * detar_y_u / detar_x_u \
                    + 2 / Re * detar_x_u / detar_y_u

    else:
        a_e[y, x] = detar_x_u * detar_y_u / detar_t \
                + detar_y_u * (max((uu[y, x+1]+uu[y,x+2])/2, 0) + max(-(uu[y, x+1]+uu[y,x])/2, 0)) \
                + detar_x_u * (max((vv[y+1, x+1]+vv[y+1,x])/2, 0) + max(-(vv[y, x+1]+vv[y,x])/2, 0)) \
                + 2 / Re * detar_y_u / detar_x_u \
                + 2 / Re * detar_x_u / detar_y_u

    a_w[y, x] = detar_x_u * detar_y_u / detar_t \
                + detar_y_u * (max((uu[y, x]+uu[y,x+1])/2, 0) + max(-(uu[y, x]+uu[y,x-1])/2, 0)) \
                + detar_x_u * (max((vv[y+1, x]+vv[y+1,x-1])/2, 0) + max(-(vv[y, x]+vv[y,x-1])/2, 0)) \
                + 2 / Re * detar_y_u / detar_x_u \
                + 2 / Re * detar_x_u / detar_y_u

    if y == n-1 and x!= n-1:
        a_n[y, x] = detar_x_v * detar_y_v / detar_t \
                + detar_x_v * (max(-(vv[y,x])/2, 0)) \
                + detar_y_v * (max((uu[y, x+1])/2, 0) + max(-(uu[y, x])/2, 0)) \
        ...
        a_n[y, x] = detar_x_v * detar_y_v / detar_t \
                    + detar_x_v * (max(-(vv[y, x]) / 2, 0)) \
                    + detar_y_v * (max(-(uu[y, x]) / 2, 0)) \
                    + 2 / Re * detar_y_v / detar_x_v \
                    + 2 / Re * detar_x_v / detar_y_v
    else:
        a_n[y, x] = detar_x_v * detar_y_v / detar_t \
                + detar_x_v * (max((vv[y+2, x]+vv[y+1,x])/2, 0) + max(-(vv[y+1, x]+vv[y,x])/2, 0)) \
                + detar_y_v * (max((uu[y, x+1]+uu[y+1, x+1])/2, 0) + max(-(uu[y+1, x]+uu[y, x])/2, 0)) \
                + 2 / Re * detar_y_v / detar_x_v \
                + 2 / Re * detar_x_v / detar_y_v

    a_s[y, x] = detar_x_v * detar_y_v / detar_t \
                + detar_x_v * (max((vv[y+1, x]+vv[y,x])/2, 0) + max(-(vv[y, x]+vv[y-1,x])/2, 0)) \
                + detar_y_v * (max((uu[y-1, x+1]+uu[y, x+1])/2, 0) + max(-(uu[y, x]+uu[y-1, x])/2, 0)) \
                + 2 / Re * detar_y_v / detar_x_v \
                + 2 / Re * detar_x_v / detar_y_v
...
def u_add_solution (x, y, i_u, j_u, i_v, j_v, E, W, N, S, detar_x, detar_y, Re, uu, vv, alpha_u, alpha_v, alpha_p):
    ...
    u[y, x] = uu[y, x] + d[y, x - 1] * (p_add[y, x - 1] - p_add[y, x])
...
def v_add_solution (x, y, i_u, j_u, i_v, j_v, E, W, N, S, detar_x, detar_y, Re, uu, vv, alpha_u, alpha_v, alpha_p):
    ...
    v[y, x] = vv[y, x] + d[y - 1, x] * (p_add[y - 1, x] - p_add[y, x])
